...
with open("C:\Users\Beast\Documents\IISc\Research\logs\oct-14\\10k_f.log") as f:
    skip = 0
    old = 0
    for line in f:
        l = line[:-1].split(",")
        ...
        if len(l) == 7:
            ...
            if skip > 1:
                ...
                if l[4] == 'Superstep:-1':
                    # Start new query
                    temp[query_no+'-'+query_split] = [query_type, etime, etime, msg, 0]
                    old = query_no+'-'+query_split
                else:
                    # Update query stats
                    temp[old][2] = etime
                    temp[old][3] += msg
        elif len(l) == 5:
            if l[-1].split(':')[0] == 'estimatedCost':
                query_no = l[1].split(':')[-1]
                query_split = l[2].split(":")[1]
                temp1[query_no+'-'+query_split] = float(l[-1].split(":")[1])
...
for i in temp:
    lines.append([i.split("-")[0], i.split("-")[1], temp[i][0], (temp[i][2] - temp[i][1])/1000.0, temp1[i]])

with open('10k_F_split_times.csv', 'w') as writeFile:
