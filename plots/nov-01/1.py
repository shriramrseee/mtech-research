...
with open("C:\Users\Beast\Documents\IISc\Research\logs\\nov-02\\10k_dw_coeff.log") as f:
    ...
    for line in f:
        ...
        if len(l) == 7:
            ...
            jlr1 = l[9].split(':')[-1]
            jc1 = l[10].split(':')[-1]
            ct1 = float(l[11].split(':')[-1])/1000000.0
            ct2 = float(l[12].split(':')[-1])/1000000.0
            cm1 = l[13].split(':')[-1]
            st1 = float(l[14].split(':')[-1])/1000000.0
            st2 = float(l[15].split(':')[-1])/1000000.0
            sm1 = l[16].split(':')[-1]
            sm2 = l[17].split(':')[-1]
            sc1 = l[18].split(':')[-1]
            query_comp[query_no+'-'+split+'-'+ss] = [jt1, jl1, jr1, jlr1, jc1, ct1, ct2, cm1, st1, st2, sm1, sm2, sc1]
        elif len(l) == 14:
            query_no = l[1].split(':')[-1]
            query_type = l[2].split(":")[1]
            split = l[3].split(':')[-1]
...
#     lines.append(temp[i])
#
with open('10k_dw_coeff.csv', 'wb') as writeFile:
